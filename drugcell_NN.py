...
class drugcell_nn(nn.Module):
    ...
    def __init__(self, term_size_map, term_direct_gene_map, dG, ngene, ndrug, root, num_hiddens_genotype, num_hiddens_drug, num_hiddens_final, activation):
        ...
        self.gene_dim = ngene   # 3008         
        self.drug_dim = ndrug   # 1024 or 2048, morgan fingerprint dim
        ...
        self.mode = None
        self.cell_emb_path = None
        self.drug_emb_path = None
    ...
    def forward(self, x, term_act=None):
        gene_input = x.narrow(1, 0, self.gene_dim)  # [None, 3008]
        drug_input = x.narrow(1, self.gene_dim, self.drug_dim)  # [None, 1024]

        # define forward function for genotype dcell #############################################
        term_gene_out_map = {}  # [None, term_size]
        ...
        for i, layer in enumerate(self.term_layer_list):
            ...
            for term in layer:
                ...
                Tanh_out = self.activation_fun(term_NN_out)
                term_NN_out_map[term] = self._modules[term+'_batchnorm_layer'](Tanh_out)
                aux_layer1_out = self.activation_fun(self._modules[term+'_aux_linear_layer1'](term_NN_out_map[term]))
                print(self.activation_fun)
                term_act[term+'_aux_linear_layer1_activation'] = aux_layer1_out.detach().cpu().numpy()
        ...
        for i in range(1, len(self.num_hiddens_drug)+1, 1):
            drug_out = self._modules['drug_batchnorm_layer_'+str(i)]( self.activation_fun(self._modules['drug_linear_layer_' + str(i)](drug_out)))
            term_NN_out_map['drug_'+str(i)] = drug_out

            aux_layer1_out = self.activation_fun(self._modules['drug_aux_linear_layer1_'+str(i)](drug_out))
            aux_out_map['drug_'+str(i)] = self._modules['drug_aux_linear_layer2_'+str(i)](aux_layer1_out)       

        # connect two neural networks at the top #################################################
        if self.mode == 'interpret':
            torch.save(term_NN_out_map[self.root], self.cell_emb_path)
            torch.save(drug_out, self.drug_emb_path)
        final_input = torch.cat((term_NN_out_map[self.root], drug_out), 1)

        out = self._modules['final_batchnorm_layer'](self.activation_fun(self._modules['final_linear_layer'](final_input)))
        term_NN_out_map['final'] = out

        aux_layer_out = self.activation_fun(self._modules['final_aux_linear_layer'](out))
