...
def train_model(root, term_size_map, term_direct_gene_map, dG, train_data, gene_dim, drug_dim, model_save_folder, train_epochs, batch_size, learning_rate, num_hiddens_genotype, num_hiddens_drug, num_hiddens_final, cell_features, drug_features,activation):
    ...
    for epoch in range(train_epochs):
        ...
        train_predict_label = (torch.sigmoid(train_predict) >= 0.5).float()
        train_corr = (train_predict_label == train_label_gpu).float().mean()
        ...
        test_predict_label = (torch.sigmoid(test_predict) >= 0.5).float()
        test_corr = (test_predict_label == test_label_gpu).float().mean()
