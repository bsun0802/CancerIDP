...
def train_model(root, term_size_map, term_direct_gene_map, dG, train_data, gene_dim, drug_dim, model_save_folder, train_epochs, batch_size, learning_rate, num_hiddens_genotype, num_hiddens_drug, num_hiddens_final, cell_features, drug_features, activation):
    ...
    model = drugcell_nn(term_size_map, term_direct_gene_map, dG, gene_dim, drug_dim, root, num_hiddens_genotype, num_hiddens_drug, num_hiddens_final, activation)
    ...
    for epoch in range(train_epochs):
        ...
        print("epoch\t%d\tcuda_id\t%d\ttrain_corr\t%.6f\tval_corr\t%.6f\ttotal_loss\t%.6f\telapsed_time\t%s" % (epoch, CUDA_ID, train_corr, test_corr, total_loss, epoch_end_time-epoch_start_time) + f'\tpred_min\t{test_predict.min().item():.3f}\tpred_max\t{test_predict.max().item():.3f}\tgt_min\t{test_label_gpu.min().item():.3f}\tgt_max{test_label_gpu.max().item():.3f}')
...
parser.add_argument('-activation', help='non-linear activation', type=str, default='tanh')
...
train_model(root, term_size_map, term_direct_gene_map, dG, train_data, num_genes, drug_dim, opt.modeldir, opt.epoch, opt.batchsize, opt.lr, num_hiddens_genotype, num_hiddens_drug, num_hiddens_final, cell_features, drug_features, opt.activation)
