...
def predict_dcell(predict_data, gene_dim, drug_dim, model_file, hidden_folder, batch_size, result_file, cell_features, drug_features):
    ...
    cell_emb_path = '/scratch1/bos/bos/DepMap-output/alive_patient_livability_pred_v2/result/cell_emb.pt'
    drug_emb_path = '/scratch1/bos/bos/DepMap-output/alive_patient_livability_pred_v2/result/drug_emb.pt'
    model = torch.load(model_file, map_location='cuda:%d' % CUDA_ID)
    if opt.interpret:
        model.mode = 'interpret'
        model.cell_emb_path = cell_emb_path
        model.drug_emb_path = drug_emb_path
    ...
    test_accuracy = (test_predict_label ==  predict_label_gpu).float().mean()
    print("Test accuracy \t%s\t%.6f" % (model.root, test_accuracy))

    np.savetxt(result_file+'/drugcell_binary.predict', test_predict_label.cpu().numpy(),'%i')
