...
def predict_dcell(predict_data, gene_dim, drug_dim, model_file, hidden_folder, batch_size, result_file, cell_features, drug_features):
    ...
    for i, (inputdata, labels) in enumerate(test_loader):
        ...
        aux_out_map, term_hidden_map = model(cuda_features, term_activatinos)
        if opt.interpret:
            term_act_file = os.path.join(os.path.join(result_file, 'term_activation.pkl'))
            with open(term_act_file, 'wb') as f:
                pickle.dump(term_activatinos, f)
